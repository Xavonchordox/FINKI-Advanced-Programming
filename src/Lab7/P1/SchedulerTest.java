...
class Scheduler<T> {
    private TreeMap<Date, T> mapByDate;
    public Scheduler() {
        mapByDate = new TreeMap<>();
    }
    ...
    public T next() {
        Date now = new Date();
        return mapByDate.ceilingEntry(now).getValue();
    }

    public T last() {
        Date now = new Date();
        return mapByDate.floorEntry(now).getValue();
    }

    public ArrayList<T> getAll(Date begin, Date end) {
        return new ArrayList<T>(mapByDate.subMap(begin, end).values());
    }

    public T getFirst() {
        return mapByDate.firstEntry().getValue();
    }

    public T getLast() {
        return mapByDate.lastEntry().getValue();
    }
}
