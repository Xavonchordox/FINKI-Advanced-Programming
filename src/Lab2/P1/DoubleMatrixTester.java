...
class DoubleMatrix {
    ...
    public DoubleMatrix(double[] array, int n, int m) throws InsufficientElementsException {
        if (array.length < m*n) {
            throw new InsufficientElementsException();
        }

        matrix = new double[n][m];
        int k = array.length - (m*n);
    ...
    double maxElementAtColumn(int column) throws InvalidColumnNumberException {
        ...
        return Arrays.stream(matrix).mapToDouble(doubles -> doubles[column - 1])
                .max().orElse(0);
    }
    ...
    double[] toSortedArray() {
        double[] sort = Arrays.stream(matrix)
                .flatMapToDouble(Arrays::stream)
                .sorted()
                .toArray();

        return IntStream.range(0, sort.length)
                .mapToDouble(i -> sort[sort.length - 1 - i])
                .toArray();
    }
...
class MatrixReader {
    public static DoubleMatrix read(InputStream input) throws InsufficientElementsException {
        ...
        return new DoubleMatrix(arr, row, col);
    }
}
